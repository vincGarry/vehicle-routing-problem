...
def get_matrix(coordinates: str) -> dict:
    ...
    try:
        ...
        url = f"http://10.36.10.16:5000/table/v1/driving/{coordinates}?annotations=distance"
        # print(url)
...
def parse_coordinates2(dictRequest:dict) -> str:
    coordinates = [f"{data["long"]},{data["lat"]}" for data in dictRequest]
    return ";".join(coordinates)
